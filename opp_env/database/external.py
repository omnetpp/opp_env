def get_project_descriptions():
    return [
...

        {
            ...
            "git_url": "https://github.com/sfc-aqua/quisp.git",
            "patch_commands": [
                r"""git reset --hard accc4946b70abf608505ec3e60301d151121c25a""",    # the releases need git, so we're using a commit of the master branch
            ],
            "setenv_commands": [r"""export OMNETPP_IMAGE_PATH=$QUISP_ROOT/quisp/images:$OMNETPP_IMAGE_PATH""",
                                r"""echo 'Hint: in the quisp folder, use the `./quisp` command to run simulations. For example: `./quisp simulations/two_nodes.ini`'"""],
            "build_commands": [r"""make eigen && make json && make spdlog && make IMAGE_PATH=quisp/images/ -j$NIX_BUILD_CORES MODE=$BUILD_MODE"""],
...

        {
            ...
            "download_commands": [
                ...
                r"""mkdir swim-src""",
                r"""cd swim-src""",
                r"""curl -L -o swim.tar.gz https://github.com/ComNets-Bremen/SWIMMobility/archive/e319b2ae5f1038d61bda42dac37d6c4e1c9d40c1.tar.gz --progress-bar""",
                r"""tar -xzf swim.tar.gz --strip=1""",
                r"""rm swim.tar.gz""",
...

        {
            ...
            "patch_commands": [
                ...
                r"""sed -i -E 's|--no-deep-includes||' Makefile""",
...

        {
            ...
            "download_url": "https://github.com/inet-framework/openflow/archive/refs/tags/v20240124.tar.gz",
            "patch_commands": [
                r"""sed -i -E 's|-KINET_PROJ=[^ ]+|-KINET_PROJ=$(INET_ROOT) -o OpenFlow|' Makefile""",
                r"""sed -i 's|$DIR/../../inet|$INET_ROOT|' src/run_openflow""",
                r"""sed -i 's|opp_run_dbg|opp_run|' src/run_openflow""",
...

        {
            ...
            "smoke_test_commands": [
                ...
                r"""cd simulations""",
                r"""../src/inbaversim$DBG_POSTFIX omnetpp-advanced-iot-network.ini -n .:$INBAVERSIM_ROOT/src:$INET_ROOT/src -u Cmdenv --sim-time-limit=1000s"""
             ],
        },
...

	{
...

        {
            ...
            "smoke_test_commands": [
                ...
                r"""if [ "$BUILD_MODE" = "release" ]; then export INET_GPTP_VALIDATION_PAPER_BIN=$(echo $GPTP_HOWHANGLIU2024_PAPER_ROOT/workspace/network_validation/out/*-release/src/validation_paper); fi""",
                r"""if [ "$BUILD_MODE" = "debug" ]; then export INET_GPTP_VALIDATION_PAPER_BIN=$(echo $GPTP_HOWHANGLIU2024_PAPER_ROOT/workspace/network_validation/out/*-debug/src/validation_paper_dbg); fi""",
...

        {
            ...
            "patch_commands": [
                r"""sed -i 's|../../inet|$(INET_ROOT)|g' Makefile""",
                r"""sed -i 's|--deep|--deep -o 6gdetcom|g' Makefile""",
                r"""sed -i 's|../../../deterministic6g_data|../../deterministic6g_data|g' */*/omnetpp.ini""",
            ],
            "setenv_commands": [
                r"""export INET_ROOT=$SIXGDETCOM_ALLINONE_ROOT/inet""",
                r"""cd $INET_ROOT && . setenv""",
                r"""echo 'Hint: In an example simulation folder, use the `$SIXGDETCOM_ALLINONE_ROOT/src/6gdetcom -n ..:$SIXGDETCOM_ALLINONE_ROOT/src:$INET_ROOT/src` command to run the simulation. Note: several example simulations require .xml files in the `deterministic6g_data` folder. These can be generated by `main.py` scripts in that folder.'""",
            ],
            "smoke_test_commands": [
                r"""cd simulations/cooperativemanufacturing""",
                r"""if [ "$BUILD_MODE" = "release" ]; then DEBUG_SUFFIX=""; fi""",
                r"""if [ "$BUILD_MODE" = "debug" ]; then DEBUG_SUFFIX="_dbg"; fi""",
                r"""$SIXGDETCOM_ALLINONE_ROOT/src/6gdetcom$DEBUG_SUFFIX -n ..:$SIXGDETCOM_ALLINONE_ROOT/src:$INET_ROOT/src -c OnlyAgvToCell -u Cmdenv --sim-time-limit=1s"""
            ],
            "download_commands": [
                r"""mkdir sixgdetcom_allinone-20250910 && cd sixgdetcom_allinone-20250910""",
                r"""curl -L -o src.tar.gz https://github.com/DETERMINISTIC6G/6GDetCom_Simulator/archive/7d3a08ba2eeab687e135f60f27f1bc79dacff153.tar.gz --progress-bar""",
                r"""tar -xzf src.tar.gz --strip=1""",
                r"""rm src.tar.gz""",
                r"""curl -L -o src.tar.gz https://github.com/DETERMINISTIC6G/inet-gptp/archive/389db4ae319b0b066c501e91c236e96bcad87e53.tar.gz --progress-bar""",
                r"""mkdir inet && tar -xzf src.tar.gz --strip=1 -C inet""",
                r"""rm src.tar.gz""",
                r"""curl -L -o src.tar.gz https://github.com/DETERMINISTIC6G/deterministic6g_data/archive/694effc0cffafb711d43a46b3abffb9a5d1af303.tar.gz --progress-bar""",
                r"""tar -xzf src.tar.gz""",
                r"""rm src.tar.gz""",
                r"""mv deterministic6g_data-* deterministic6g_data""",
            ],
            "build_commands": [
                r"""cd inet""",
                "OPP_FEATURETOOL=\"$OMNETPP_ROOT/src/utils/opp_featuretool\"",
                r"""$OPP_FEATURETOOL -v enable Z3GateSchedulingConfigurator VoipStream VoipStreamExamples TcpLwip""",
        ...
        {
            # builds in release
            "name": "leosatellites", "version": "2.0.0",
            "required_projects": {"omnetpp": ["6.0.*"], "inet": ["4.4.0"], "os3": ["20220928"]},
            "description": "",
            "details": """""",
            "nix_packages": [
                "libxml2",
            ...
            "patch_commands": [
                "find src -name '*.cc' -exec sed -i 's/std::__fs::filesystem::/std::filesystem::/g' {} +",
            ],
            "setenv_commands": [
                r"""export IGRAPH_INCLUDE=${pkgs.igraph.dev}/include/igraph""",
                # r"""export CURL_INCLUDE=${pkgs.curl}/include""",
                r"""export LIBXML2_INCLUDE=${pkgs.libxml2}/include""",
                "export LIBXML2_LIB=${pkgs.libxml2}/lib",
                r"""export GMP_LIB=${pkgs.gmp}/lib""",
                r"""export BLAS_LIB=${pkgs.blas}/lib""",
                r"""export GLPK_LIB=${pkgs.glpk}/lib""",
                r"""export LAPACK_LIB=${pkgs.lapack}/lib""",
                r"""export ARPACK_LIB=${pkgs.arpack}/lib""",
                "export XML_LIB=${pkgs.libxml2}/lib",
                
                r"""echo 'Hint: In an example simulation folder, use the `$SIXGDETCOM_ALLINONE_ROOT/src/6gdetcom -n ..:$SIXGDETCOM_ALLINONE_ROOT/src:$INET_ROOT/src` command to run the simulation. Note: several example simulations require .xml files in the `deterministic6g_data` folder. These can be generated by `main.py` scripts in that folder.'""",
            ],
            "smoke_test_commands": [
            ],
            "download_url": "https://github.com/Avian688/leosatellites/archive/refs/tags/v2.0.0.tar.gz",
            # "download_url": "https://github.com/Avian688/leosatellites/archive/15f6832a815ead81bcaf807f34d309b030cdc7e2.tar.gz",
            "build_commands": [
                "cd src && opp_makemake --make-so -f --deep -o leosatellites -I$IGRAPH_INCLUDE -I$OS3_ROOT/src -I$INET_ROOT/src -I$CURL_INCLUDE -L$INET_ROOT/src -L$OS3_ROOT/src -L$GMP_LIB -L$BLAS_LIB -L$GLPK_LIB -L$LAPACK_LIB -L$ARPACK_LIB -ligraph -lxml2 -lz -lgmp -lblas -lglpk -llapack -larpack -lINET\$D -los3\$D",
                r"""make -j$NIX_BUILD_CORES MODE=$BUILD_MODE CFLAGS='-std=c++17 -fPIC'""",
             ],
            "clean_commands": [r"""cd inet && make clean MODE=$BUILD_MODE && cd .. && make clean MODE=$BUILD_MODE"""],
        },
    ]
