...
def make_veins_project_description(veins_version, inet_versions, sumo_version, omnetpp_versions):
    ...
    return {
        ...
        "clean_commands": [ "[ ! -f src/Makefile ] || make cleanall MODE=$BUILD_MODE && cd subprojects/veins_inet && [ ! -f src/Makefile ] || make cleanall MODE=$BUILD_MODE && cd ../veins_inet3 && [ ! -f src/Makefile ] || make cleanall MODE=$BUILD_MODE" ],
        "options": {
            "from-source-archive": {
...
def get_project_descriptions():
    return [ make_veins_project_description(veins_version, inet_versions, sumo_version, omnetpp_versions) for veins_version, inet_versions, sumo_version, omnetpp_versions in [
        ...
        # note for 5.2:
        ["5.1", ["4.2.1", "4.2.0", "3.6.5", "3.6.6", "3.6.7", "3.6.8"], ["1.9.2"], ["5.5.2", "5.5.1", "5.4.*", "5.3.*"]],
        # note for 5.1:
